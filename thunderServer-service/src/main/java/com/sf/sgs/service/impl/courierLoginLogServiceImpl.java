...
/**
 * courierLoginLogService实现类
 *
 * @author 594829
*/
@Service
public class courierLoginLogServiceImpl implements courierLoginLogService {

	private Logger logger = LoggerFactory.getLogger(getClass());
			
	@Autowired
	courierLoginLogManager courierLoginLogManager;
    ...
    */
	public Integer countByExample(courierLoginLogExample example) {
		int count = 0;
		try{
			count = courierLoginLogManager.countByExample(example);
		}
		catch(Exception e){
    ...
    */
    public Integer deleteByPrimaryKey(Long id){
		...
		int count = 0;
		try{
			count = courierLoginLogManager.deleteByPrimaryKey(id);
		}
		catch(Exception e){
	 ...
	 *
    */
    public Integer insert(courierLoginLog record){
		if(record == null){
		...
		int count = 0;
		try{
			count = courierLoginLogManager.insert(record);
		}
		catch(Exception e){
	 ...
	 *
    */
    public Page<courierLoginLog> selectByExample(courierLoginLogExample example, Integer pageNum, Integer pageSize){
		...
		Page<courierLoginLog> records = new Page<courierLoginLog>();
		PageHelper.startPage(pageNum, pageSize);
		try{
			records = (Page<courierLoginLog>)courierLoginLogManager.selectByExample(example);
		}
		catch(Exception e){
    ...
    */
	public courierLoginLog selectByPrimaryKey(Long id){
		...
		courierLoginLog record = null;
		try{
			record = courierLoginLogManager.selectByPrimaryKey(id);
		}
		catch(Exception e){
	 ...
	 *
    */
	public Integer updateByPrimaryKeySelective(courierLoginLog record){
		if(record == null){
			throw new IllegalStateException("参数record不能为空");
		}
    	if(record.getId() == null){
		...
		int count = 0;
		try{
			count = courierLoginLogManager.updateByPrimaryKeySelective(record);
		}
		catch(Exception e){
	 ...
	 *
    */
    public Integer updateByPrimaryKey(courierLoginLog record){
		if(record == null){
			throw new IllegalStateException("参数record不能为空");
		}
    	if(record.getId() == null){
		...
		int count = 0;
		try{
			count = courierLoginLogManager.updateByPrimaryKey(record);
		}
		catch(Exception e){
